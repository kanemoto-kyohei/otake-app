...
foreach ($carender_elements['datetimes'] as $datetime){
  ...
  for ($col = 1; $col < 8; $col++) {
      ...
      foreach ($appoints as $appoint) {
              if ($appoint['date'] == $formattedDate && $appoint['time'] == $datetime->gettime()) {
                  $is_appoint = true;
                  break;
          }
          if ($is_appoint) break;
      ...
      if (weekday_find($col,$carender_elements['weekdays'])) {
          echo "x";
      } elseif (holiday_find($datesimple, $carender_elements['holiday']) == "true") {
          echo "x";
      } elseif ($is_appoint) {
          echo $appoint['user']['name'];
      } else {
          echo "◎";
      }
      echo "</td>";
...
<!--カレンダーここまで-->
<form action = "{{ route('admin.reset',['permalink' => $permalink]) }}" method = "post">
  @csrf
  <button type = "submit" value = "カレンダーを再設定する">カレンダーの設定に戻る</button>
</form>
<br>
